...
foreach ($relation in $relations)
{
	if ($monthly -eq $true)
	{
		...
		if ($relDP)
		{
			...
			if ($relDP.PolicyType -eq "vault")
			{
				...
				$Policy=Get-NcSnapmirrorPolicy -PolicyName $relDP.policy
				if ($Policy.SnapmirrorPolicyRules.snapmirrorlabel -contains $weeklysnaplabel -and $Policy.SnapmirrorPolicyRules.snapmirrorlabel -contains $monthlysnaplabel -and $Policy.SnapmirrorPolicyRules.snapmirrorlabel -contains $yearlysnaplabel)
				{
					...
					$monthlysnap=New-NcSnapshot -Volume $relDP.sourcevolume -VserverContext $relDP.sourcevserver -Snapshot $MonthlySnapName -SnapmirrorLabel $monthlysnaplabel
					...
					if ($yearly -eq $true)
					{
						...
						$yearlysnap=New-NcSnapshot -Volume $relDP.sourcevolume -VserverContext $relDP.sourcevserver -Snapshot $YearlySnapName -SnapmirrorLabel $yearlysnaplabel
		...
		else
		{
			...
			$MonthlySnapName="Monthly_"+$date.month.tostring()+"_"+$rndnumber.ToString()
			$monthlysnap=New-NcSnapshot -Volume $relation.sourcevolume -VserverContext $relation.sourcevserver -Snapshot $MonthlySnapName -SnapmirrorLabel $monthlysnaplabel
			Logwrite "Monthly snapshot name = $($monthlysnap.name)"
			if ($yearly -eq $true)
			{
				Logwrite "It is the first sunday of the year. Taking a yearly snapshot"
				$YearlySnapName="Yearly_"+$date.year.tostring()+"_"+$rndnumber.ToString()
				$yearlysnap=New-NcSnapshot -Volume $relation.sourcevolume -VserverContext $relation.sourcevserver -Snapshot $YearlySnapName -SnapmirrorLabel $yearlysnaplabel
				Logwrite "Yearly snapshot name = $($yearlysnap.name)"
			}
	...
	if ($weekly -eq $true)
	{
		...
		if ($relDP)
		{
			...
			$weeklysnap=New-NcSnapshot -Volume $relDP.sourcevolume -VserverContext $relDP.sourcevserver -Snapshot $WeeklySnapName -SnapmirrorLabel $weeklysnaplabel
			...
			if ($monthly -eq $true)
			{
				...
				if ($NewSMstate.NewestSnapshot -ne $monthlysnap.name)
				{
					if ($vault -eq $true)
					{
						...
						Remove-NcSnapshot -Volume $relDP.sourcevolume -VserverContext $relDP.sourcevserver -Snapshot $monthlysnap.name -IgnoreOwners -confirm:$false |out-null
						$CurrentNcController = $null
			...
			if ($yearly -eq $true)
			{
				...
				if ($NewSMstate.NewestSnapshot -ne $yearlysnap.name)
				{
					if ($vault -eq $true)
					{
						...
						Remove-NcSnapshot -Volume $relDP.sourcevolume -VserverContext $relDP.sourcevserver -Snapshot $yearlysnap.name -IgnoreOwners -confirm:$false |out-null
						$CurrentNcController = $null
		...
		else
		{
			Logwrite "It is Sunday. Taking a Weekly snapshot"
			$WeeklySnapName="Weekly_"+$date.day.tostring()+"_"+$rndnumber.ToString()
			$weeklysnap=New-NcSnapshot -Volume $relation.sourcevolume -VserverContext $relation.sourcevserver -Snapshot $WeeklySnapName -SnapmirrorLabel $weeklysnaplabel
			Logwrite "Force replication to : $($relation.DestinationLocation) to push newly weekly snapshots"
			Invoke-NcSnapmirrorUpdate -Destination $relation.DestinationLocation |out-null
			Logwrite "Let's check replication status"
			$snapmirrored = $false
